...
function scrollToBottom(div) {
  ...
  console.log(height);
  chat_body.scrollIntoView(false);
  //div.scrollTop = height;
  //div.scrollTop.behavior = 'smooth';
};

// анимация для плавного появления текста
function textAppearing (idOfSpan, activeTab, i) {
  return new Promise((resolve) => {
    setTimeout(() => {
      tabs_storage[activeTab].messages[idOfSpan].firstChild.firstChild.firstChild.children[i].style = `animation: appearing ${i * 0.1}s linear`;
      resolve("resolved");
    }, 2000);
  });
};

async function textChanging (idOfSpan, activeTab, i) {
  let result = await textAppearing(idOfSpan, activeTab, i);
}

// плавное появление сообщений
function appearing(div, text) {
  let textContent = div.firstChild.firstChild.firstChild.textContent;
  console.log(textContent);
  tabs_storage[document.getElementsByClassName('active-tab')[0].firstChild.firstChild.textContent].messages.push(div);
};
...
$("#delete-tab").click(function(event) {
  ...
  let tabsName = tab.firstChild.firstChild.textContent;

  // запрет на удаление единственной вкладки
  if (side_bar.children.length != 2) {
    console.log(side_bar.children.length)
    side_bar.removeChild(tab);
    delete tabs_storage[tabsName];
  } else {
...
function message_el (message, class_name) {
    ...
    if (class_name == "chat-bot") {
      chat_content = `<div class="code-solvation"><pre><code class="hljs python-html">${message}</code></pre></div>`;
...
function manage_chat() {
  ...
  if (stored_info) {
    let userMessage = message_el(user_message, "user");
    chat_body.appendChild(userMessage);



    stored_info.messages.push(userMessage);
    tabs_storage[current_active_tab] = stored_info;

    //scrollToBottom(userMessage);

  //   chat_body.scrollIntoView({
  //     behavior: 'smooth'
  //   });
  }
  ...
  var botMessage = message_el("...", "chat-bot");
  botMessage.style = "animation: appearing 0.3s linear";
  stored_info.messages.push(botMessage);
  chat_body.appendChild(botMessage);
  setTimeout(() => botMessage.style = "", 1000);

  var idOfBotMessage = chat_body.children.length - 1;
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      var botMessageNew = message_el(solution, "chat-bot");
      ...
      if (stored_info) {
        //stored_info.messages[idOfBotMessage].firstChild.firstChild.firstChild.innerHTML = '';
        //botMessage.style = "animation: appearing 1s linear";
        stored_info.messages[idOfBotMessage] = botMessageNew;
        chat_body.replaceChild(botMessageNew, botMessage);
      };
      ...
      $.ajax({
        ...
        success: function (data) {
          for (let i = 0; i < data.tests.length; i++) {
            ...
            if (stored_info) {
              let test = create_test(input = data.tests[i].input, output = data.tests[i].expected, solution = solution);
              stored_info.tests.push(test);
              tabs_storage[current_active_tab] = stored_info;
              test_list.insertBefore(test, add_test);
            }
          }
        }
      });
...
$(document).ready(function(){
  ...
  // var asd = message_el('asdwasd', 'chat-bot');
  // appearing(asd);
});

function textAppearing (idOfSpan, activeTab, i) {
  tabs_storage[activeTab].messages[idOfSpan].firstChild.firstChild.firstChild.children[i].style = "animation: appearing 1s";
  //stored_info.messages[idOfSpan].firstChild.firstChild.firstChild.children[i].style = "animation: appearing 1s linear";
};
